const DEFAULT_STATE = {
    ...
    history: [],
    achievements: {
        first_focus_hour: { name: "First Steps", desc: "Earn 1 hour of focus time", unlocked: false },
        five_day_streak: { name: "Assistant to the Regional Manager", desc: "5-day streak", unlocked: false },
        level_5: { name: "Leveling Up", desc: "Reach level 5", unlocked: false },
        buy_item: { name: "Retail Therapy", desc: "Purchase your first item", unlocked: false },
        focus_master: { name: "Focus Master", desc: "Accumalate 10 hours total", unlocked: false }
    },
...
async function gameTick() {
    ...
    if (cat == "productive") {
        ...
        if (newXp >= state.xpToNextLevel) {
            ...
            newLevel += 1;
            newXpToNext = Math.floor(state.xpToNextLevel * 1.4);
        }
        ...
        logEntry = { time: Date.now(), msg: "Productive session", delta: `+${fpGain} FP`, type: "gain" };
...
async function checkNewDay() {
    ...
    const history = [...(state.history || []), histEntry].slice(-7);
...
async function updateCurrentTab(url) {
    ...
    if (state.productiveSites.some(s => hostname.includes(s))) category = "productive";
...
async function checkAchievements() {
    ...
    for (const [key, condition] of Object.entries(checks)) {
        if (condition && achievements[key] && !achievements[key].unlocked) {
            achievements[key] = { ...achievements[key], unlocked: true, unlockedAt: Date.now() };
            changed = true;
        }
    }
    if (changed) await chrome.storage.local.set({ achievements });
}
...
async function handleMessage(msg) {
    ...
    switch (msg.type) {
        ...
        case "BUY_ITEM": {
            ...
            const achievements = { ...state.achievements };
            if (achievements.buy_item) achievements.buy_item.unlocked = true;
            await chrome.storage.local.set({
